...
// .wrangler/tmp/bundle-qug9sD/middleware-loader.entry.ts
init_modules_watch_stub();
init_performance2();

// .wrangler/tmp/bundle-qug9sD/middleware-insertion-facade.js
init_modules_watch_stub();
init_performance2();
...
// worker/src/extractRecipe.js
init_modules_watch_stub();
init_performance2();

// worker/src/normalizeRecipe.js
init_modules_watch_stub();
init_performance2();

// worker/src/unitMap.js
init_modules_watch_stub();
init_performance2();
var UNIT_MAP = {
  tbsp: "tablespoon",
...
__name(normalizeAmount, "normalizeAmount");
function isEmptyIngredient(ingredient) {
  ...
  return ["name", "amount", "unit"].every((field) => {
    ...
    return value === void 0 || value === null || trimString(value) === "";
  });
}
__name(isEmptyIngredient, "isEmptyIngredient");
...
function normalizeSteps(steps) {
  ...
  return stepList.filter((step) => step !== void 0 && step !== null).map((step) => String(step).trim()).filter((step) => step.length > 0);
}
...
function normalizeNotes(notes) {
  ...
  return noteList.filter((note) => note !== void 0 && note !== null).map((note) => String(note).trim()).filter((note) => note.length > 0);
}
...
function normalizeRecipe(recipe) {
  ...
  if (Object.hasOwn(recipe, "ingredients")) {
    ...
    normalizedRecipe.ingredients = ingredients.filter((ingredient) => !isEmptyIngredient(ingredient)).map(normalizeIngredient);
  }
...
__name(normalizeRecipe, "normalizeRecipe");
...
var recipe_schema_default = {
  ...
  properties: {
    ...
    notes: {
      type: "array",
      description: "Operational notes, source context, or special handling details for kitchen production.",
      items: {
        type: "string",
        description: "A single recipe note."
      }
    }
  }
};
...
function parseAmount(value) {
  ...
  const text = String(value || "").trim().replace("\xBC", "1/4").replace("\xBD", "1/2").replace("\xBE", "3/4").replace("\u2153", "1/3").replace("\u2154", "2/3").replace("\u215B", "1/8").replace("\u215C", "3/8").replace("\u215D", "5/8").replace("\u215E", "7/8");
  if (!text) {
    return 0;
  }
...
function parseIngredientText(rawIngredient) {
  ...
  const match = text.match(new RegExp(`^([\\d./\\s\xBC\xBD\xBE\u2153\u2154\u215B\u215C\u215D\u215E-]+)?\\s*(${INGREDIENT_UNIT_PATTERN})?\\b\\s*(.*)$`, "i"));
...
__name(parseIngredientText, "parseIngredientText");
...
__name(mapIngredient, "mapIngredient");
function getJsonLdBlocks(html) {
  return [...html.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)].map((match) => stripHtml(match[1])).filter(Boolean).flatMap((jsonText) => {
    try {
...
__name(getJsonLdBlocks, "getJsonLdBlocks");
...
__name(mapInstruction, "mapInstruction");
...
__name(getFirstTagText, "getFirstTagText");
function extractListByClassHint(html, hints) {
  ...
  return [...html.matchAll(itemPattern)].map((match) => stripHtml(match[1])).filter((value) => value.length > 0);
}
__name(extractListByClassHint, "extractListByClassHint");
function extractRecipeFromHtml(html, sourceUrl = "") {
  const jsonLdRecipes = getJsonLdBlocks(html).flatMap(flattenJsonLd).find(isRecipeNode);
  if (jsonLdRecipes) {
    return {
      ...
      steps: (Array.isArray(jsonLdRecipes.recipeInstructions) ? jsonLdRecipes.recipeInstructions : [jsonLdRecipes.recipeInstructions]).map(mapInstruction).filter(Boolean),
      notes: [
...
__name(extractRecipeFromHtml, "extractRecipeFromHtml");
function extractRecipeFromHtmlDocument(html, sourceUrl = "") {
  return normalizeRecipe(extractRecipeFromHtml(html, sourceUrl));
}
__name(extractRecipeFromHtmlDocument, "extractRecipeFromHtmlDocument");

// worker/src/recipePatchApi.js
init_modules_watch_stub();
init_performance2();
var ALLOWED_PATCH_FIELDS = ["title", "yield", "category", "ingredients", "steps", "notes"];
...
var ADMIN_EXTRACT_URL_PATH = "/api/admin/extract-url";
...
__name(handleExtractRecipeUrl, "handleExtractRecipeUrl");
